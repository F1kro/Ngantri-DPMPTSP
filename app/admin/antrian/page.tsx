...
const QueueTimer = ({
  ...
  if (timeLeft === null)
    return <div className="text-2xl font-mono text-slate-700">--:--</div>;
  ...
  return (
    <div className="flex items-center gap-2 text-4xl font-black font-mono text-emerald-400 px-6 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl shadow-[0_0_20px_rgba(16,185,129,0.1)]">
      <Clock size={24} /> {String(min).padStart(2, "0")}:
...
export default function ManajemenAntrean() {
  ...
  const fetchData = async () => {
    ...
    const sortedServices = (serviceRes.data || []).sort((a, b) => {
      const countA = (bookingRes.data || []).filter(
        (bk) => bk.service_id === a.id && bk.status !== "completed",
      ).length;
      const countB = (bookingRes.data || []).filter(
  ...
  const activeQueue = useMemo(
    () =>
      bookings.find(
        (b) => b.status === "in_progress" && b.service_id === selectedServiceId,
      ),
  ...
  const handleAction = async (type: "NEXT" | "SELESAI" | "ULANG") => {
    if (!selectedServiceId)
      return toast.warning("Pilih jenis layanan terlebih dahulu");

    if (type === "NEXT") {
      const next = bookings.find(
        (b) => b.status === "waiting" && b.service_id === selectedServiceId,
      );
      ...
      <div className="flex-1 p-6 space-y-6 flex flex-col h-full relative">
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-[2rem] p-5 shadow-2xl flex items-center justify-between gap-6 shrink-0 z-10">
          ...
          {activeQueue && (
            <QueueTimer
              ...
              durationMinutes={activeQueue.services?.estimated_duration || 15}
            />
          )}
        ...
        <div className="flex items-center gap-3 shrink-0 px-1">
          <Button
            ...
            variant="outline"
            className="h-12 w-12 bg-slate-900 border-slate-800 rounded-xl hover:bg-indigo-600"
          >
            <ChevronLeft />
          </Button>
                ...
                <button
                  ...
                  className={`relative h-12 rounded-xl font-black uppercase text-[9px] border transition-all px-3 ${
                    selectedServiceId === s.id
                      ? "bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-600/40 scale-105 z-20"
        ...
        <div className="flex-1 grid grid-cols-2 gap-6 min-h-0 overflow-hidden">
          <div className="bg-slate-900/40 border border-slate-800 rounded-[2rem] flex flex-col overflow-hidden shadow-xl">
            <div className="p-4 border-b border-slate-800 bg-indigo-600/5 flex justify-between items-center">
              <h3 className="font-black uppercase text-[10px] text-indigo-400">
                Daftar Tunggu
              </h3>
                ...
                <tbody>
                  {waitingList
                    ...
                    .map((b) => (
                      <tr
                        key={b.id}
                        className={`border-b border-slate-800/50 ${b.status === "in_progress" ? "bg-indigo-600/10" : ""}`}
                      >
                        <td className="p-4 font-mono font-black text-indigo-400 text-lg">
                          {b.booking_number}
                        </td>
                        <td className="p-4 text-xs font-bold uppercase truncate max-w-[150px] text-slate-200">
                          {b.visitor_name}
                        </td>
                        <td className="p-4">
                          <Badge
                            className={`text-[8px] font-black ${b.status === "in_progress" ? "bg-emerald-500 text-slate-950" : "bg-slate-800 text-slate-400"}`}
                          >
            ...
            <div className="p-3 border-t border-slate-800 flex justify-between items-center bg-slate-950/50">
              <Button
                ...
                size="sm"
                className="h-8 w-8 rounded-lg bg-slate-900 border-slate-800"
              >
                <ChevronLeft size={14} />
              </Button>
              ...
              <Button
                disabled={pageWaiting * itemsPerWaiting >= waitingList.length}
                onClick={() => setPageWaiting((p) => p + 1)}
                size="sm"
                className="h-8 w-8 rounded-lg bg-slate-900 border-slate-800"
              >
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-[2rem] flex flex-col overflow-hidden shadow-xl">
            <div className="p-4 border-b border-slate-800 bg-slate-950 flex justify-between items-center">
              <h3 className="font-black uppercase text-[10px] text-slate-500">
                Riwayat Layanan
              </h3>
            </div>
            <div className="flex-1">
                    ...
                    .map((b) => (
                      ...
                      >
                        ...
                        <td className="p-4 text-center">
                          <Badge
                            variant="outline"
                            className={`text-[8px] font-black border-none ${b.status === "completed" ? "text-emerald-500 bg-emerald-500/10" : "text-slate-600 bg-slate-800/50"}`}
                          >
            ...
            <div className="p-3 border-t border-slate-800 flex justify-between items-center bg-slate-950/50">
              ...
              <span className="text-[9px] font-black text-slate-600 tracking-tighter">
                PAGE {pageHistory}
              </span>
              <Button
