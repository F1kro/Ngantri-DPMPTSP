                ...
                <form onSubmit={handleAdd} className="space-y-6 pt-6">
                  <div className="grid grid-cols-3 gap-4">
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Kode</label>
                      <Input
                        placeholder="A"
                        maxLength={2}
                    ...
                    <div className="col-span-2 space-y-3">
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Nama Layanan</label>
                      <Input
              ...
              <div className="flex items-end gap-2.5">
                <div className="w-[280px]">
                  <label className="flex items-center gap-1.5 text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1 mb-1.5">
                    <Search size={11} className="text-indigo-400"/> Cari Layanan / Kode
                  </label>
                  <Input 
            ...
            <div className="overflow-auto flex-1 custom-scrollbar">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-800/40">
                  {loading ? (
                    <tr><td colSpan={4} className="py-20 text-center"><Loader2 className="animate-spin mx-auto text-indigo-500" size={24} /></td></tr>
                      ...
                      <tr key={s.id} className="hover:bg-indigo-500/[0.02] transition-colors group">
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-3">
                            <div className="p-2 bg-slate-950 rounded-lg border border-slate-800 text-indigo-400"><Briefcase size={16} /></div>
      ...
      <Dialog open={openEdit} onOpenChange={setOpenEdit}>
        <DialogContent className="bg-[#0f172a] border-slate-800 text-white rounded-[2.5rem] p-10 shadow-2xl [&>button]:hidden">
          ...
          <form onSubmit={handleUpdate} className="space-y-6 pt-6">
             <div className="grid grid-cols-3 gap-4">
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Kode</label>
                  <Input className="bg-slate-950 border-slate-800 h-16 rounded-2xl text-white font-black text-center text-2xl uppercase" value={editService.prefix_code} onChange={(e) => setEditService({ ...editService, prefix_code: e.target.value })} required />
                </div>
                <div className="col-span-2 space-y-3">
                  <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Nama Layanan</label>
                  <Input className="bg-slate-950 border-slate-800 h-16 rounded-2xl text-white font-bold text-xl uppercase" value={editService.name} onChange={(e) => setEditService({ ...editService, name: e.target.value })} required />
                </div>
             </div>
            <div className="space-y-3">
        ...
        <AlertDialogContent className="bg-[#0f172a] border-slate-800 text-white rounded-[2rem] p-8 shadow-2xl">
          <AlertDialogHeader className="space-y-4">
            ...
            <div className="text-center space-y-2">
              ...
              <AlertDialogDescription className="text-slate-400">Yakin hapus layanan ini? Riwayat antrean mungkin terdampak. Tindakan ini <span className="text-red-400 font-bold underline uppercase">permanen</span>.</AlertDialogDescription>
            </div>
          </AlertDialogHeader>
