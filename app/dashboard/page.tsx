...
// Komponen Timer Mandiri - Diperbaiki untuk Deployment
const MonitorTimer = ({
  startTime,
  durationMinutes,
}: {
  ...
  useEffect(() => {
    const calculate = () => {
      ...
      // Menggunakan getTime() untuk perbandingan milidetik yang absolut
      const start = new Date(startTime).getTime();
      const end = start + durationMinutes * 60000;
  ...
  // Menghindari Hydration Mismatch: Tampilkan placeholder jika waktu belum terhitung
  if (timeLeft === null) return <div className="text-3xl md:text-5xl font-mono text-slate-700">--:--</div>;
